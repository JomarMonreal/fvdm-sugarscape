...
def run_one_simulation(args):
    ...
    Returns a small status dictionary so the parent process can parse logs
    immediately and show stderr if a simulation crashes.
    """
    condition_name, seed, config_path, log_path, agent_log_path, python_alias = args
...
def run_focal_action(args):
    ...
    parsed_agent_logs = set()

    print(f"  Target per-action observations: {target_per_action}")
    print(f"  Actions: {DISC_ACTIONS}")
    print(f"  Strategy: run up to {num_cores} simulations concurrently; stop after a completed batch satisfies target\n")
    ...
    def add_observations_from_log(cname, seed, agent_log_path):
        """Parse one completed agent log once and update tallies."""
        if agent_log_path in parsed_agent_logs:
            return 0
        obs = parse_agent_log(agent_log_path, cname, seed)
        parsed_agent_logs.add(agent_log_path)
        if not obs:
            return 0
    ...
    # Build all per-seed configs first. With one bias condition, this is what
    # allows --cores to matter: the pool can run different seeds concurrently.
    jobs = []
    for seed_idx, seed in enumerate(seeds):
        for condition_name, decision_models in BIAS_CONDITIONS.items():
            ...
            jobs.append((condition_name, seed, cfg_path, cfg["logfile"], cfg["agentLogfile"], python_alias))

    # Reuse completed logs if they exist; only dispatch missing/bad logs.
    pending_jobs = []
    for cname, seed, cfg_path, log_path, agent_log_path, py in jobs:
        if os.path.exists(agent_log_path):
            agent_data = safe_json_load(agent_log_path)
            if agent_data and len(agent_data) > 0:
                add_observations_from_log(cname, seed, agent_log_path)
                if all_actions_satisfied():
                    break
                continue
        pending_jobs.append((cname, seed, cfg_path, log_path, agent_log_path, py))

    if all_actions_satisfied():
        print(f"  ✓ Target already satisfied from existing logs: {action_status_str()}")
    elif pending_jobs:
        total_pending = len(pending_jobs)
        completed = 0

        # Run in chunks of size num_cores so early stopping can happen between
        # chunks without launching all remaining seeds unnecessarily.
        for chunk_start in range(0, total_pending, num_cores):
            if all_actions_satisfied():
                print(f"\n  ✓ All actions satisfied. Stopping early before launching more simulations.")
                break

            chunk = pending_jobs[chunk_start:chunk_start + num_cores]
            print(f"  Launching simulations {chunk_start + 1}-{chunk_start + len(chunk)} of {total_pending} "
                  f"on {min(num_cores, len(chunk))} core(s) ...")

            with multiprocessing.Pool(processes=min(num_cores, len(chunk))) as pool:
                for result in pool.imap_unordered(run_one_simulation, chunk):
                    completed += 1
                    cname = result["condition"]
                    seed = result["seed"]
                    agent_log_path = result["agent_log_path"]
                    cfg_base = os.path.basename(result["config_path"])

                    if result["returncode"] != 0:
                        print(f"\n  [error] Simulation failed: {cfg_base}")
                        if result["stderr"]:
                            print(result["stderr"])
                        continue

                    added = add_observations_from_log(cname, seed, agent_log_path)
                    pct = completed / total_pending * 100
                    print(f"\r  [{completed:>4}/{total_pending}] {pct:5.1f}%  "
                          f"added={added:<7} {action_status_str()}  last={cfg_base:<45}",
                          end="", flush=True)
            print()
