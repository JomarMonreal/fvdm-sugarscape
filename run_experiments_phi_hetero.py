#!/usr/bin/env python3
"""
run_experiments_phi_hetero.py
------------------------------
Experiment 6: Heterogeneous Egoist–Bentham sweep using φ-welfare FVDM agents.

Sweeps fvdmPhiEgoist (φ=1.0) + fvdmPhiBentham (φ=0.5) proportions from 0% to
100% Bentham in HETERO_STEP (default 10) percentage-point increments → 11 conditions.

These agents use the Bentham welfare formula for cell selection (survival-viable)
and log v_imm / v_fut for each chosen cell, enabling BFS measurement.

This is the direct FVDM counterpart to the baseline hetero sweep in
run_experiments_baseline.py. The key question: does increasing the proportion
of φ=0.5 (Bentham-like) FVDM agents produce the same societal outcome gradient
as the original Bentham baseline agents?

Requires:
  baseline_results/hetero_results/per_seed_felicific.csv  (for BFS vs baseline)

Outputs (under fvdm_phi_results/hetero_results/):
  per_seed_summary.csv     — extinction, final pop, Gini, TTL per seed
  condition_aggregates.csv — mean ± sd across seeds per proportion level
  hetero_bfs.csv           — BFS(phi-hetero, baseline-hetero) per proportion × seed
  hetero_spearman.csv      — Spearman rho between pct_bentham and each outcome metric

Usage:
  python run_experiments_phi_hetero.py
  python run_experiments_phi_hetero.py -s 30 -t 5000 -a 250 -j 30
  python run_experiments_phi_hetero.py --hetero-step 10 --force
"""
...
HETERO_STEP = 10
OUTPUT_DIR  = os.path.join("fvdm_phi_results", "hetero_results")
...
def make_models_list(pct_bentham: int, total_agents: int = 250) -> list:
    if pct_bentham == 0:
        return ["fvdmPhiEgoist"]
    if pct_bentham == 100:
        return ["fvdmPhiBentham"]
    n_bentham = round(pct_bentham / 100 * total_agents)
    n_egoist  = total_agents - n_bentham
    g = gcd(n_bentham, n_egoist)
    return ["fvdmPhiEgoist"] * (n_egoist // g) + ["fvdmPhiBentham"] * (n_bentham // g)
...
def make_hetero_conditions(step: int = HETERO_STEP):
    ...
    for pct in range(0, 101, step):
        fname             = f"phiHeteroMix_p{pct:03d}"
...
def parse_agent_log(agent_log_path: str, condition: str, seed: int):
    """Aggregate v_imm_* / v_fut_* over all rows with neighbours > 0."""
    ...
    for row in rows:
        if int(row.get("neighbors", row.get("neighbourhood", 0))) <= 0:
            continue
        try:
            imm = np.array([float(row.get(f"v_imm_{l}", 0.0)) for l in IMM_LABELS])
            fut = np.array([float(row.get(f"v_fut_{l}", 0.0)) for l in FUT_LABELS])
            imm_acc += imm
            fut_acc += fut
            n += 1
        except Exception:
...
def compute_bfs_vs_baseline(felicific_rows: list, base_pairs: dict,
    ...
    if not os.path.exists(baseline_hetero_csv):
        ...
        print(f"         Run run_experiments_baseline.py first to enable BFS computation.")
        return []
    ...
    for frow in felicific_rows:
        ...
        phi_imm  = np.array(frow["mu_imm"])
        phi_fut  = np.array(frow["mu_fut"])
        ...
        cos_imm = cosine_sim(phi_imm, base_imm)
        cos_fut = cosine_sim(phi_fut, base_fut)
...
def main(args):
    ...
    print(f"\n{'='*65}")
    print(f"  φ-Welfare FVDM Heterogeneous Egoist–Bentham Sweep (Exp 6)")
    print(f"{'='*65}")
    print(f"  Agents: fvdmPhiEgoist (φ=1.0) + fvdmPhiBentham (φ=0.5)")
    print(f"  Decision: argmax φ·h_self(c) + (1−φ)·h_neighbors(c)  [survives]")
    ...
    if total > 0:
        ...
        lock    = mgr.Lock()
        pool_args = [(cfg, python, counter, lock, total) for cfg, _ in run_queue]
...
def parse_args():
    p = argparse.ArgumentParser(
        description="φ-welfare FVDM heterogeneous Egoist-Bentham sweep (Exp 6)",
    ...
    p.add_argument("--baseline-dir",     default="baseline_results",
                   help="Directory containing hetero_results/per_seed_felicific.csv "
