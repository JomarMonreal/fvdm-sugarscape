...
where certainty ($c$) and propinquity ($p$) weigh the overall outcome, immediate rewards are captured by extent ($e$), intensity ($i$), and duration ($d$), and future rewards use future intensity ($i_f$) and future duration ($d_f$) discounted by $\gamma$ to represent uncertainty in long-term predictions. When $\gamma = 0$, the future term vanishes and the formula reduces to $h = cp \cdot e(i+d)$.
...
\textbf{Temporal discount} $\gamma$ is a configured lookahead factor that scales the future welfare term. When lookahead is disabled, $\gamma = 0$ and the entire future term drops out. In all experiments conducted in this study, \texttt{decisionModelLookaheadFactor} is set to zero, which disables the future term entirely and reduces the operative formula to:
\[
h = cp \cdot e(i + d)
\]
The full expression $h = cp\bigl[e(i+d) + \gamma e_f(i_f+d_f)\bigr]$ is preserved in the specification for completeness and to maintain fidelity to the original KH2024 formulation, but $\gamma e_f(i_f+d_f) = 0$ holds throughout all reported simulations. The future-layer coordinates $i_f$ and $d_f$ are therefore defined but inactive in the baseline decision rule.
